Add-Type @"
    ...
    public class WinApi
    {
        ...
        public class User32
        {
            ...
            //[DllIMport("user32.dll")] public static extern int    MapWindowPoints(IntPtr hWndFrom, IntPtr hWndTo, out POINT lpRect, uint cPoints);
            //[DllIMport("user32.dll")] public static extern IntPtr SetWindowLongPtrW(IntPtr hWnd, int nIndex, IntPtr dwNewLong); //return value LONG_PTR
            //[DllIMport("user32.dll")] public static extern IntPtr GetWindowLongPtrW(IntPtr hWnd, int nIndex); //return value LONG_PTR
    ...
    public class HandleStackedWindows
    {
        ...
        private const int HK_PIN_CURRENT_WND      = 3;
        private const int HK_UNPIN_CURRENT_WND    = 4;
        ...
        static public void PinWindow(IntPtr hWnd){
            WinApi.RECT rect;
            WinApi.User32.GetWindowRect(hWnd, out rect);
            WinApi.User32.SetWindowPos(hWnd, WinApi.HWND_TOPMOST, 0, 0, rect.right - rect.left, rect.bottom - rect.top, WinApi.SWP_NOMOVE);
        }
        static public void UnpinWindow(IntPtr hWnd){
            WinApi.RECT rect;
            WinApi.User32.GetWindowRect(hWnd, out rect);
            WinApi.User32.SetWindowPos(hWnd, WinApi.HWND_NOTOPMOST, 0, 0, rect.right - rect.left, rect.bottom - rect.top, WinApi.SWP_NOMOVE);
        }
        static public void Dispose(int hkNum){
            switch(hkNum){
                ...
                case HK_PIN_CURRENT_WND:
                {
                    PinWindow(WinApi.User32.GetForegroundWindow());
                    break;
                }
                case HK_UNPIN_CURRENT_WND:
                {
                    UnpinWindow(WinApi.User32.GetForegroundWindow());
                    break;
                }
            }
        }
...
Set-Variable VK_OEM_5     -Option Constant -Value 0x00DC
...
Set-Variable HK_PIN_CURRENT_WND      -Option Constant -Value 0x0003
Set-Variable HK_UNPIN_CURRENT_WND    -Option Constant -Value 0x0004
...
if([WinApi+User32]::RegisterHotKey(0, $HK_CLOSE_STACKED_WND, $MOD_WIN    -Bor $MOD_NOREPEAT, $VK_ESC)       -And
   ...
   [WinApi+User32]::RegisterHotKey(0, $HK_RESTORE_STACKED_WND, $MOD_WIN  -Bor $MOD_SHIFT -Bor $MOD_NOREPEAT, $VK_OEM_MINUS) -And
   [WinApi+User32]::RegisterHotKey(0, $HK_PIN_CURRENT_WND, $MOD_WIN -Bor $MOD_SHIFT -Bor $MOD_NOREPEAT, $VK_OEM_5) -And
   [WinApi+User32]::RegisterHotKey(0, $HK_UNPIN_CURRENT_WND, $MOD_WIN -Bor $MOD_NOREPEAT, $VK_OEM_5)){
    ...
    Write-Host("Press 'WIN+SHIFT+|' to pin current window.")
    Write-Host("Press 'WIN+\' to unpin current window.")
}else{
